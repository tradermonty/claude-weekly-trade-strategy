...
def screen_value_dividend_stocks(fmp_api_key: str, top_n: int = 20,
    ...
    for i, stock in enumerate(candidates, 1):  # Analyze all candidates until rate limit
        ...
        # Store RSI for later filtering (will filter after all analysis)
        stock['_rsi'] = rsi
    ...
    if oversold_results:
        ...
        final_results = oversold_results[:top_n]
    else:
    ...
    print(f"Ranking complete. Returning {len(final_results)} stocks.", file=sys.stderr)
