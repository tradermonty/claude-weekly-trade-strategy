...
def fetch_csv(url: str, timeout: int = 30) -> list:
    ...
    req = urllib.request.Request(
        url, headers={"User-Agent": "breadth-csv-fetcher/1.0"}
    )
...
def format_human(result: AnalysisResult) -> str:
    ...
    cross_desc = "8MA ABOVE -- NO dead cross" if not result.dead_cross else "8MA BELOW -- DEAD CROSS"
    lines.append(
        f"8MA vs 200MA: {cross_sign}{result.cross_diff:.2f}pt ({cross_desc})"
    )
    ...
    lines.append(
        f"Current: {result.uptrend_ratio:.2f}% {result.uptrend_color} "
        f"({result.uptrend_class})"
    )
    lines.append(
    ...
    if result.sectors:
        ...
        overbought = [
            s for s in result.sectors
            if s.get("status", "").lower() == "overbought"
        ]
        oversold = [
            s for s in result.sectors
            if s.get("status", "").lower() == "oversold"
        ]
        uptrend_sectors = [
            s for s in result.sectors
            if s.get("trend", "").lower() in ("up", "uptrend")
        ]
        downtrend_sectors = [
            s for s in result.sectors
            if s.get("trend", "").lower() in ("down", "downtrend")
        ]
